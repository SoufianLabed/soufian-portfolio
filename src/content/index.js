export default {
  project:{
    links: [
      [{ img:   process.env.PUBLIC_URL + '/assets/chess.jpg', name: 'Chess game',description:'Object-oriented implementation of a chess game. This project was realized in duo.', git:"https://github.com/SoufianLabed/Jeu_echec" },
      { img:  process.env.PUBLIC_URL + '/assets/paint.png', name: 'Paint',description:'Development of an image manipulation interface using Python library: Abstract Window Toolkit. Solo project.', git:"https://github.com/SoufianLabed/Paint" },],
      [
        { img:   process.env.PUBLIC_URL + '/assets/othello.png', name: 'Othello game',description:'Implementation of an Othello-like game made in duo.', git:"https://github.com/SoufianLabed/Othello" },
        { img:  process.env.PUBLIC_URL + '/assets/udp_tcp.png', name: 'UDP and TCP Server',description:'Development of UDP and TCP servers using socket and sys libraries.', git:"https://github.com/SoufianLabed/serverUDP-TCP" },],
        [
          { img:   process.env.PUBLIC_URL + '/assets/ireceptio.png', name: 'iReception',description:'Development of an order reception module synchronized to the Dolibarr ERP software using PHP, javascript and MySQL.', git:"" },
          ],

          [
            { img:   process.env.PUBLIC_URL + '/assets/Hikomori.png', name: 'Hikikomori',description:'Design and development of a japanese library web app in reactjs and nodejs. Team of 4  ', git:"https://github.com/damien77640/hikomori_front" },
            { img:  process.env.PUBLIC_URL + '/assets/pokedex.jpg', name: 'Pokedex',description:'School project : creation of a website in react using a server express ', git:"https://github.com/SoufianLabed/cx-react-pokemon" },
            { img:  process.env.PUBLIC_URL + '/assets/port.png', name: 'Personnal Web Site',description:'Design and development of my personal portfolio. This project was done solo', git:"https://github.com/SoufianLabed/portoflio-Soufian" },],
            [
              { img:   process.env.PUBLIC_URL + '/assets/maths.png', name: 'Calcul App Game',description:'Developpement of a Maths calcul Game in Android. Solo' , git:"https://github.com/SoufianLabed/CalculAndroidApp"},] 

    ]
  },
  test: {
    test: 'D:/Users/sosol/Desktop/react/mon-app/src/img/right-arrow.png',
  },
    nav: {
      logo: 'CC',
      links: [
        { img:  process.env.PUBLIC_URL + '/assets/user.png', to: 'header' },
        { img:  process.env.PUBLIC_URL + '/assets/code.png', to: 'Stack' },
        { img:  process.env.PUBLIC_URL + '/assets/diplome.png', to: 'code' },
        { img:  process.env.PUBLIC_URL + '/assets/malette.png', to: 'code' },
      ],
    },
    header: {
      imgPlaceholder:
        'data:image/jpeg;base64,',
      text: ['Soufian LABED', "Bachelor Student Conceptor & Developer at EFREI Paris", 'Project Manager Assistant for Saint-Gobain'],
      typical: [
        'web developer. 🖥',
        2000,
        'Youtuber. 📹',
        2000,
        'Mobile Developer',
        2000,
      ],
      btnText: 'Discover More',
    },
    work: {
      title: 'Projects',
      img: process.env.PUBLIC_URL + '/assets/project-mobile.png',
      imgPlaceholder:
        'data:image/png;base64,',
      projectName: 'Sleeping',
      desc:
        'peaceful, and tranquil mean quiet and free from disturbance. calm is used when someone is not excited or upset even when there is cause for it. They stayed calm during the fire. peaceful is used when someone or something has reached a quiet state after some period of disturbance.',
    },
    stack: {
      title: 'Stack',
      tech: [
        [
          {
            img: process.env.PUBLIC_URL + '/assets/php.png',
            alt: 'java',
            value: 70
          },
          {
            img: process.env.PUBLIC_URL + '/assets/python.png',
            alt: 'react',
            value: 65
          },
          {
            img: process.env.PUBLIC_URL + '/assets/java.png',
            alt: 'react',
            value: 80
          },
          {
            img: process.env.PUBLIC_URL + '/assets/mysql.png',
            alt: 'react',
            value: 85
          },
        ]
        ,
      [
        {
          img: process.env.PUBLIC_URL + '/logo512.png',
          alt: 'react',
          value: 90
        },
        {
          img: process.env.PUBLIC_URL + '/assets/node.png',
          alt: 'react',
          value: 75
        },
        {
          img: process.env.PUBLIC_URL + '/assets/mongo.png',
          alt: 'react',
          value: 70
        },
        {
          img: process.env.PUBLIC_URL + '/assets/android.png',
          alt: 'react',
          value: 70
        },
      ]],
      desc: `Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only`,
    },
    contact: {
      title: 'Ping Me!',
      desc:
        "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book.",
      socials: [
        {
          alt: 'facebook',
          img: process.env.PUBLIC_URL + '/assets/facebook.png',
        },
        {
          alt: 'github',
          img: process.env.PUBLIC_URL + '/assets/github.png',
        },
        {
          alt: 'link',
          img: process.env.PUBLIC_URL + '/assets/link.png',
        },
        {
          alt: 'gmail',
          img: process.env.PUBLIC_URL + '/assets/gmail.png',
        },
      ],
      img:
        'https://images.unsplash.com/photo-1555952517-2e8e729e0b44?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=1300&q=80',
      imgPlaceholder:
        'data:image/jpeg;base64,',
    },
  };
  